...
angular.module('app.controllers', ['pascalprecht.translate', 'ngCookies'])
  .controller('AppCtrl', ['$scope', '$translate', '$localStorage', '$location', function($scope, $translate, $localStorage, $location) {
    ...
    $scope.users = [
        {
            email: 'jeremyfroment@typiak.fr',
            password: 'password',
            firstname: 'Jérémy',
            name: 'Froment',
            type: 'Administrateur',
            image: 'https://yt3.ggpht.com/-TrCfkfwbz5Y/AAAAAAAAAAI/AAAAAAAAAAA/dy9oeSbpwaE/s100-c-k-no/photo.jpg'
        },
        {
            email: 'arnaudbastide@typiak.fr',
            password: 'password',
            firstname: 'Arnaud',
            name: 'Bastide',
            type: 'Administrateur',
    ...
    $scope.submit = function() {
        ...
        angular.forEach ($scope.users, function($user, $key) {
            if ($scope.user.email == $user.email && $scope.user.password == $user.password) {
                ...
                switch ($scope.app.user.type) {
                    case 'Administrateur' :
                        $localStorage.settings.navbarCollapseColor = "bg-dark";
                        $localStorage.settings.navbarHeaderColor   = "bg-dark";
                        $localStorage.settings.navbarAsideColor    = "bg-dark";
                        $localStorage.settings.firstColor          = '#3a3f51';
                        $localStorage.settings.secondColor         = '#3a3f51';
                        $localStorage.settings.nativeFirstColor    = 'dark';
                        $localStorage.settings.nativeSecondColor   = 'dark';
                        break;
                    case 'Gerant' :
                        $localStorage.settings.navbarCollapseColor = "bg-success";
                        $localStorage.settings.navbarHeaderColor   = "bg-success";
                        $localStorage.settings.navbarAsideColor    = "bg-dark";
                        $localStorage.settings.firstColor          = '#27c24c';
                        $localStorage.settings.secondColor         = '#3a3f51';
                        $localStorage.settings.nativeFirstColor    = 'success';
                        $localStorage.settings.nativeSecondColor   = 'dark';
                        break;
                    case 'Adherant' :
                        $localStorage.settings.navbarCollapseColor = "bg-danger";
                        $localStorage.settings.navbarHeaderColor   = "bg-danger";
                        $localStorage.settings.navbarAsideColor    = "bg-dark";
                        $localStorage.settings.firstColor          = '#f05050';
                        $localStorage.settings.secondColor         = '#3a3f51';
                        $localStorage.settings.nativeFirstColor    = 'danger';
                        $localStorage.settings.nativeSecondColor   = 'dark';
                        break;
                    case 'Employe' :
                        $localStorage.settings.navbarCollapseColor = "bg-info";
                        $localStorage.settings.navbarHeaderColor   = "bg-info";
                        $localStorage.settings.navbarAsideColor    = "bg-dark";
                        $localStorage.settings.firstColor          = '#23b7e5';
                        $localStorage.settings.secondColor         = '#3a3f51';
                        $localStorage.settings.nativeFirstColor    = 'info';
                        $localStorage.settings.nativeSecondColor   = 'dark';
                        break;
                    case 'Benevole' :
                        $localStorage.settings.navbarCollapseColor = "bg-primary";
                        $localStorage.settings.navbarHeaderColor   = "bg-primary";
                        $localStorage.settings.navbarAsideColor    = "bg-dark";
                        $localStorage.settings.firstColor          = '#7266ba';
                        $localStorage.settings.secondColor         = '#3a3f51';
                        $localStorage.settings.nativeFirstColor    = 'primary';
                        $localStorage.settings.nativeSecondColor   = 'dark';
                        break;
                }
        ...
        if ($scope.app.user == null && $localStorage.user == null) {
